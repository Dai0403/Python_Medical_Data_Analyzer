...
df['overweight'] = df.apply(lambda row: determine_bmi(row['bmi']), axis = 1)"""
# print(df['overweight'])
# or df['overweight'] = df['bmi'].apply(determine_bmi)
...
def draw_cat_plot():
  ...
  # you have to show the value too-> in this case you use not sum() but size() to count the number of columns for each variable
